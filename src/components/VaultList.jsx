...
export default function VaultList({ onSelectItem, onAddNew }) {
    ...
    <div className="max-w-4xl mx-auto px-4 py-4 pb-24">
      ...
      <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-3 mb-2">
        {categories.map((cat) => {
          ...
          const CatIcon = cat.id === 'favorites' ? Star : (ICON_MAP[cat.id] || Shield);
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-all duration-150 shrink-0 ${
                isActive
                  ? 'bg-emerald-500 text-surface-950 font-bold shadow-md shadow-emerald-950/60'
      ...
      {sortedItems.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-2">
          {sortedItems.map((item) => {
            const CatIcon = ICON_MAP[item.category] || Shield;
              ...
              >
                ...
                <div className="flex items-start justify-between gap-2 mb-3">
                  <div className="flex items-center gap-2.5">
                    <div className="w-10 h-10 rounded-xl bg-surface-900 border border-slate-700/60 flex items-center justify-center text-emerald-400 group-hover:border-emerald-500/40 transition-colors">
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-slate-400 capitalize">
                        {settings.categories.find(c => c.id === item.category)?.name || item.category}
                      </span>
                    </div>
                  </div>
